...
var getFsCacheKey = function(absoluteInputPaths) {
   ...
   return $s.digest(absoluteInputPaths.concat(modifyTimes).join('')) + fs.extension(absoluteInputPaths[0]);
};
...
CompressorNode.prototype.renderCompressed = function(context) {
   ...
   if (cacheContent !== null) {
      log.trace('Cache HIT', ehCacheKey);
      return cacheContent;
   }
   ...
   if (this.env.config.compressor.cache) {
      ...
      log.trace('Cache MISS', ehCacheKey);
   }
